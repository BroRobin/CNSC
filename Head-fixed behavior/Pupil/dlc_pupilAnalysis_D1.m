...
for zz=1:15
    ...
    if zz==5 || zz==7
        ...
        for ii=1:size(P.fileList,2)
            % for ii=10:12
            R.pupmov_location_steps_X_avg_centered(ii,:)=R.pupmov_location_steps_X_avg(ii,:)-R.pupmov_location_steps_X_avg(ii,8);
            R.pupmov_location_steps_Y_avg_centered(ii,:)=R.pupmov_location_steps_Y_avg(ii,:)-R.pupmov_location_steps_Y_avg(ii,8);
            plot(R.pupmov_location_steps_X_avg_centered(ii,[8 10]),R.pupmov_location_steps_Y_avg_centered(ii,[8 10]),'-o')
        end
        title(['Saccades 100-200ms after stimulus'])
        xlim([(nanmax(abs(R.pupmov_location_steps_X_avg_centered(:,10)))*-1)-1 (nanmax(abs(R.pupmov_location_steps_X_avg_centered(:,10))))+1])
        ylim([(nanmax(abs(R.pupmov_location_steps_Y_avg_centered(:,10)))*-1)-1 (nanmax(abs(R.pupmov_location_steps_Y_avg_centered(:,10))))+1])
    ...
    if zz==5 || zz==7
        for ii=1:size(R.pupmov_location_steps_X_avg_centered,1)
            idx = [R.pupmov_location_steps_X_avg_centered(ii,10) R.pupmov_location_steps_Y_avg_centered(ii,10)];
    ...
    for ii = 1:size(P.fileList,2)
        ...
        S.pupmov_amp_100ms(ii,1) = nanmax(S.pupmov(ii,(0.5*P.fs):(0.6*P.fs))); % to 250ms after start stim
        S.pupmov_amp_50ms(ii,1) = nanmax(S.pupmov(ii,(0.5*P.fs):(0.55*P.fs))); % to 250ms after start stim
        S.pupmov_amp_200ms(ii,1) = nanmax(S.pupmov(ii,(0.5*P.fs):(0.7*P.fs))); % to 250ms after start stim
        ...
        if isnan(S.pupmov(ii,(0.5*P.fs)))==0
            ...
            S.pupmov_area_100ms(ii,1) = nansum(S.pupmov(ii,(0.5*P.fs):(0.6*P.fs)));
            S.pupmov_area_50ms(ii,1) = nansum(S.pupmov(ii,(0.5*P.fs):(0.55*P.fs)));
            S.pupmov_area_200ms(ii,1) = nansum(S.pupmov(ii,(0.5*P.fs):(0.7*P.fs)));
        elseif isnan(S.pupmov(ii,(0.5*P.fs)))==1
            S.pupmov_area(ii,1)=NaN;
            S.pupmov_area_baseline(ii,1) = NaN;
            S.pupmov_amp_100ms(ii,1) = NaN;
            S.pupmov_amp_50ms(ii,1) = NaN;
            S.pupmov_amp_200ms(ii,1) = NaN;
        end
    ...
    % Save the data
    P.savename = sprintf('%s_pupil_data_25Hzgauss_revised.mat',P.mouseID);
